...
*/
/*
QQuickItem* QQuickTextEdit::cursorDelegate() const
{
    Q_D(const QQuickTextEdit);
    return d->cursorComponent;
}

void QQuickTextEdit::setCursorDelegate(QQuickItem* c)
{
    Q_D(QQuickTextEdit);
...
/*void QQuickTextEdit::loadCursorDelegate()
{
    ...
    if(d->cursorComponent->isLoading())
        return;
    d->cursor = qobject_cast<QQuickItem*>(d->cursorComponent->create(qmlContext(this)));
    if(d->cursor){
        d->control->setCursorWidth(0);
        dirtyCache(cursorRectangle());
        QQuick_setParent_noEvent(d->cursor, this);
...
void QQuickTextEditPrivate::focusChanged(bool hasFocus)
{
    Q_Q(QQuickTextEdit);
    //q->setCursorVisible(hasFocus && scene && scene->hasFocus());
    //QQuickItemPrivate::focusChanged(hasFocus);
}
...
*/
/*void QQuickTextEdit::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    Q_D(QQuickTextEdit);
    if (d->focusOnPress){
...
*/
/*
void QQuickTextEdit::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    Q_D(QQuickTextEdit);
    d->control->processEvent(event, QPointF(0, -d->yoff));
...
Handles the given mouse \a event.
*/
/*void QQuickTextEdit::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_D(QQuickTextEdit);
...
Handles the given mouse \a event.
*/
/*void QQuickTextEdit::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    Q_D(QQuickTextEdit);
    d->control->processEvent(event, QPointF(0, -d->yoff));
    if (!event->isAccepted())
...
\internal
*/
/*QVariant QQuickTextEdit::inputMethodQuery(Qt::InputMethodQuery property, const QVariant &argument) const
{
    Q_D(const QQuickTextEdit);
...
the given \a bounds.
*/
/*void QQuickTextEdit::drawContents(QPainter *painter, const QRect &bounds)
{
    Q_D(QQuickTextEdit);
...
/*void QQuickTextEdit::updateImgCache(const QRectF &rf)
{
    Q_D(const QQuickTextEdit);
    QRect r;
    if (!rf.isValid()) {
...
#endif
*/
    document = new QTextDocument();//;control->document();
...
{
    Q_D(const QQuickTextEdit);
    QRectF r = QQuickItem::boundingRect();
    int cursorWidth = 1;
    if(d->cursor)
...
qreal QQuickTextEditPrivate::implicitWidth() const
{
...
void QQuickTextEdit::updateSize()
{
    ...
    /*if (isComponentComplete()) {
        ...
        if (!widthValid() && d->document->textWidth() != newWidth)
            d->document->setTextWidth(newWidth); 
        if (!widthValid())
...
void QQuickTextEdit::updatePaintNode()
{
    ...
    QTextDocument *doc = d->document->clone();
